...
const endingStyles: EndingStyle[] = [
    { value: "resolved", description: "that brings the story to a logical close" },
    { value: "cliffhanger", description: "that leaves the reader wanting more" },
    { value: "mystery", description: "that leaves the understading of the resolution up for interpretation" },
]
...
function buildPrompt(input: PromptInput): string {
    const keywords = input.additionalKeywords.split(",").map((keyword) => keyword.trim()).filter((keyword) => keyword.length > 0)
    let output = ""
    output += "Write a short story that follows the following prompt:\n"
    output += `The theme of the story should be ${input.theme}.\n`
    output += `The story should be of the ${input.genres.join(", ")} genre(s), set in the ${input.timePeriod} in ${input.location}.\n`
...
const generateStory = async (prompt: string): Promise<Story | null> => {
    const chatCompletion = await openai.chat.completions.create({
        messages: [
            { role: 'system', content: "You are an accomplished short story author who has the ability to write across all genres." },
    ...
    if (chatCompletion.choices[0].message.content) {
        try {
            ...
            console.log("Error parsing story", error, chatCompletion.choices[0].message.content)
            return null;
        }
    }
    return null
